...
export default function StudentBookPage() {
  ...
  React.useEffect(() => {
    const fetchCoaches = async () => {
      ...
      try {
        const res = await authFetch(`/api/student/coaches?clubId=${user.clubId}`)
        if (res.ok) {
  ...
  const handleSubmit = async () => {
    if (!selectedCoach || !selectedSubject || !selectedDate || !selectedTime || !selectedEndTime) return
    setSubmitting(true)
    try {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-1.5 text-sm text-gray-500">
        ...
        <span className="text-gray-900 font-medium">预约教练</span>
      </div>

      <Card>
        <CardHeader>
          <CardTitle className="text-lg">预约课程</CardTitle>
        </CardHeader>
      ...
      <Dialog open={successOpen} onOpenChange={setSuccessOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>您的预约已提交，等待教练确认。请留意通知消息。</DialogDescription>
          </DialogHeader>
          <DialogFooter>
